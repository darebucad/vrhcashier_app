          ...
          @if ($loop->first)
            <div class="input-group mb-3">
              <!-- <div class="input-group-prepend">
                <span class="input-group-text" id="basic-addon1" style="display: block;">{{ $or->or_prefix . ' - ' }}</span>
              </div> -->
              <input id="or_number" type="text" class="form-control form-control-sm" name="or_number" value="{{ $or->or_prefix . '-' . ( $or->orno + 1 ) }}" style="background-color:#99ccff!important;" required autofocus>
            </div>
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    $('#update_total').click(function(){
      ...
      // alert('clicked update total button');
        ...
        $.ajax({
          ...
          success: function(data) {
            ...
            $.each(data.data, function(i, data){
              ...
              if (discount_value == '' || discount == null || discount == 0) {
