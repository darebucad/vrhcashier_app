        ...
        <form action="{{ route('collections.outpatient.create.show') }}" method="post">
          ...
          @foreach ($outpatient_charges as $charge)
            ...
            @if ($loop->first)
              ...
              <div class="btn-group mr-2">
              <input  type="text" name="search_barcode" id="search_barcode" class="form-control form-control-sm"  value="{{ $charge->pcchrgcod }}" placeholder="Enter Barcode" autofocus>
              </div> 
  ...
  <form action="/collections/outpatient/create/payment" method="post" >
    ...
    @foreach ($outpatient_charges as $charge)
      ...
      @if ($loop->first)
        ...
        <input type="hidden" name="enccode" value="{{ $charge->enccode }}">
        <input type="hidden" name="hpercode" value="{{ $charge->hpercode }}">
        <input type="hidden" name="acctno" value="{{ $charge->acctno }}">
        <input type="hidden" name="pcchrgcod" value="{{ $charge->pcchrgcod }}">

        <div class="form-group row">
          <label for="patient_name" class="col-md-2 col-form-label text-md-left">{{ __('Patient Name') }}</label>

          <div class="col-md-6">
            <input id="patient_name" type="text" name="patient_name" class="form-control form-control-sm" value="{{ $charge->patient_name }}">
          </div>
        </div>
